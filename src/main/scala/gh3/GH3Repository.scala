...
object GH3Repository
{
   def apply(json: JValue): Option[GH3Repository] =
   {
      ...
      val created_at = node2String(json \ "updated_at").map(d => Main.formatter.parseLocalDateTime(d.take(19)))
      val updated_at = node2String(json \ "updated_at").map(d => Main.formatter.parseLocalDateTime(d.take(19))) 
      val pushed_at = node2String(json \ "updated_at").map(d => Main.formatter.parseLocalDateTime(d.take(19)))
      ...
      val homepage = (json \ "homepage") match {
         case JString(x) => Some(Some(x))
         case JNull      => Some(None)
         case _          => None     
      }
      val size = node2Long(json \ "size")
      val stargazers_count = node2Long(json \ "stargazers_count")
      val watchers_count = node2Long(json \ "watchers_count")
      val language = (json \ "language") match {
         case JString(x) => Some(Some(x))
         case JNull      => Some(None)
         case _          => None
      }
      ...
      val mirror_url = (json \ "mirror_url") match {
         case JString(x) => Some(Some(x))
         case JNull      => Some(None)
         case _          => None
      }
