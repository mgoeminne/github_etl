...
class RepositoryEventTest extends FlatSpec with Matchers
{
   "A valid RepositoryEvent" must "be correctly parsed" in {
      val json = parse(
         """
           | {
           |  "action": "created",
      ...
      RepositoryEvent(json).isDefined shouldBe true
   }
}
