...
class DeploymentStatusEventTest extends FlatSpec with Matchers
{
   "A valid DeploymentStatus event" should "be correclty parsed" in {
      val json = parse(
         """
           | {
           |  "deployment": {
      ...
      DeploymentStatusEvent(json).isDefined shouldBe true
   }
}
