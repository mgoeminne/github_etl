...
class PullRequestEventPayloadTest extends FlatSpec with Matchers
{
   "A valid PullRequestEvent payload" must "be correctly parsed" in {
      val json = parse(
         """
           ...
           |        "body":"**Bonne Année, Bon Any Nou, Buon Anno**",
           |        "changed_files":17,
   ...
   "An other valid PullRequestEvent payload" must "be correctly parsed" in {
      val json = parse(
         """
           | {
           |
   ...
   "A third valid PullRequestEvent payload" must "be correctly parsed" in {
      val json = parse(
            """
              | {
              |
